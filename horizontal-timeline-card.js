...
class HorizontalTimelineCardEditor extends HTMLElement {
    ...
    render() {
      ...
      this.shadowRoot.innerHTML = `
        ...
        <div class="card-config">
          ...
          <div class="help-text" style="margin-left: 152px;">Separar múltiples calendarios con comas</div>
          ...
          <div class="section-title">Filtros y Estadísticas</div>
          <div class="row">
            <label>Filtros Calendarios:</label>
          ...
          <div class="section-title">Iconos Automáticos</div>
          <div class="row">
            <label>Activar Auto-Iconos:</label>
            <ha-switch ${this._config.auto_icons !== false ? 'checked' : ''} configValue="auto_icons"></ha-switch>
          </div>
          <div class="help-text" style="margin-left: 152px;">Palabras: cumpleaños→🎂, reunión→👥, vacaciones→✈️, médico→🏥, etc.</div>
            ...
            ${statistics ? `
              <div class="statistics-panel">
                ...
                <div class="stat-item">
                  ...
                  <div class="stat-label">Prom/mes</div>
                </div>
              </div>
            ` : ''}
            ...
            <div class="card-wrapper ${labelPosClass} ${this._config.show_names ? "show-names" : ""}">
              <div class="timeline-wrapper">
                ...
                ${eventGroups.map(group => {
                  ...
                  return `<div class="event-group-badge" style="left: ${avgPos}%;" title="${group.events.map(e => e.event.summary).join(', ')}">${group.events.length} eventos</div>`;
                }).join('')}
                
                ${filteredEvents
                  .map(
                    (event, index) => // ${eventLevels[index]}
                      `<div class="timeline-event level-0 ${this._highlightedEventIndex === index ? 'highlighted' : ''}" 
              ...
              this._config.show_event_list && filteredEvents.length > 0
                ? `<div class="event-list-container">
                  <div class="event-list-header ${this._eventListExpanded ? "" : "collapsed"}" id="event-list-toggle">
                    ...
                    <span class="event-count">${filteredEvents.length} evento${filteredEvents.length !== 1 ? "s" : ""}</span>
                  </div>
                  <div class="event-list ${this._eventListExpanded ? "" : "collapsed"}">
                    ${filteredEvents
                      .map((event, index) => ({ ...event, originalIndex: index }))
                      // .sort((a, b) => new Date(a.start) - new Date(b.start))
                      .sort((a, b) => new Date(b.start) - new Date(a.start)) // FEcha descendente
                      .map(
                        (event) => {
  ...
  window.customCards.push({
    ...
    description: "Una tarjeta de línea de tiempo avanzada para visualizar eventos de múltiples calendarios.",
    preview: true,
